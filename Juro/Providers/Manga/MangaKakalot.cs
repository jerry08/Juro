...
namespace Juro.Providers.Manga
{
    ...
    public class MangaKakalot : IMangaProvider
    {
            ...
            CancellationToken cancellationToken = default!)
        {
            ...
            foreach (var node in nodes)
            {
                list.Add(new MangaResult()
                {
                    ...
                    HeaderForImage = new Dictionary<string, string> { { "Referer", BaseUrl } }
                });
            }
            ...
            CancellationToken cancellationToken = default!)
        {
            ...
            {
                mangaInfo.Title = document.DocumentNode.SelectSingleNode(".//div[@class='manga-info-top']/ul/li[1]/h1")
                    ?.InnerText;
                mangaInfo.AltTitles = document.DocumentNode
                                          .SelectSingleNode(".//div[@class='manga-info-top']/ul/li[1]/h2")
                                          ?.InnerText.Replace("Alternative :", "").Split(';')?.Select(x => x.Trim())
                                          .ToList() ??
                                      new List<string>();
                ...
                mangaInfo.HeaderForImage = new Dictionary<string, string> { { "Referer", BaseUrl } };
                mangaInfo.Image = document.DocumentNode.SelectSingleNode(".//div[@class='manga-info-top']/div/img")
                ...
                var statusText = document.DocumentNode.SelectSingleNode(".//div[@class='manga-info-top']/ul/li[3]")
                    ?.InnerText?.Replace("Status :", "").Trim();
                ...
                mangaInfo.Authors = document.DocumentNode.SelectNodes(".//div[@class='manga-info-top']/ul/li[2]/a")
                    ?.Select(x => x.InnerText).ToList() ?? new List<string>();
                mangaInfo.Chapters = document.DocumentNode
                    .SelectNodes(".//div[@class='chapter-list']/div[@class='row']")
                    ?.Select(el => (IMangaChapter)new MangaChapter()
                    {
                        Id = el.SelectSingleNode(".//span/a").Attributes["href"].Value
                            .Split(new[] { "chapter/" }, StringSplitOptions.None)[1],
                    ...
                    }).ToList() ?? new List<IMangaChapter>();
            }
            else
            {
                mangaInfo.Title = document.DocumentNode
                    .SelectSingleNode(".//div[@class='panel-story-info']/div[@class='story-info-right']/h1")?.InnerText;
                mangaInfo.AltTitles = document.DocumentNode
                                          .SelectSingleNode(
                                          ...
                                          .ToList() ??
                                      new List<string>();
                mangaInfo.Description = document.DocumentNode
                    .SelectSingleNode(".//div[@id='panel-story-info-description']")
                    ?.InnerText?.Replace("Description :", "").Replace(Environment.NewLine, "").Trim();
                mangaInfo.HeaderForImage = new Dictionary<string, string> { { "Referer", BaseUrl } };
                mangaInfo.Image = document.DocumentNode
                    .SelectSingleNode(".//div[@class='story-info-left']/span[@class='info-image']/img")
                    ?.Attributes["src"]?.Value;
                mangaInfo.Genres = document.DocumentNode
                    .SelectNodes(".//div[@class='story-info-right']/table/tbody/tr[4]/td[@class='table-value']/a")
                    ?.Select(x => x.InnerText).ToList() ?? new List<string>();

                var statusText = document.DocumentNode
                    .SelectSingleNode(".//div[@class='story-info-right']/table/tbody/tr[3]/td[@class='table-value']")
                    ?.InnerText?.Trim();
                mangaInfo.Status = statusText switch
                {
                    "Completed" => MediaStatus.Completed,
                ...
                mangaInfo.Views = document.DocumentNode
                    .SelectSingleNode(".//div[@class='story-info-right']/table/tbody/tr[3]/td[@class='table-value']")
                    ?.InnerText?.Trim();
                mangaInfo.Authors = document.DocumentNode
                    .SelectNodes(".//div[@class='story-info-right']/table/tbody/tr[2]/td[@class='table-value']/a")
                    ?.Select(x => x.InnerText).ToList() ?? new List<string>();
                mangaInfo.Chapters = document.DocumentNode
                    .SelectNodes(".//div[@class='container-main-left']/div[@class='panel-story-chapter-list']/ul/li")
                    ?.Select(el => (IMangaChapter)new MangaChapter()
                    {
                        Id = el.SelectSingleNode(".//a").Attributes["href"].Value
                            .Split(new[] { ".com/" }, StringSplitOptions.None)[1] + "$$READMANGANATO",
                        Title = el.SelectSingleNode(".//a").InnerText,
                        Views = el.SelectSingleNode(".//span[@class='chapter-view text-nowrap']")?.InnerText
                            ?.Replace(Environment.NewLine, "").Trim(),
                        ReleasedDate = el.SelectSingleNode(".//span[@class='chapter-time text-nowrap']")
